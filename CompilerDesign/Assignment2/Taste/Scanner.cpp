...
void Scanner::Init() {
	...
	for (i = 101; i <= 122; ++i) start.set(i, 1);
	...
	keywords.set(L"unless", 31);
	keywords.set(L"while", 32);
	keywords.set(L"for", 33);
	keywords.set(L"read", 34);
	keywords.set(L"switch", 35);
	keywords.set(L"case", 36);
	keywords.set(L"break", 37);
	keywords.set(L"write", 39);
	keywords.set(L"program", 40);
	keywords.set(L"int", 41);
	keywords.set(L"bool", 42);
	keywords.set(L"char", 43);
	keywords.set(L"const", 44);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 31:
			...
			else if (ch == L'f') {AddCh(); goto case_32;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 32:
			case_32:
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'b' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'a') {AddCh(); goto case_33;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 33:
			case_33:
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L't') || (ch >= L'v' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'u') {AddCh(); goto case_34;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 34:
			case_34:
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'k') || (ch >= L'm' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'l') {AddCh(); goto case_35;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 35:
			case_35:
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L's') || (ch >= L'u' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L't') {AddCh(); goto case_36;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 36:
			case_36:
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_1;}
